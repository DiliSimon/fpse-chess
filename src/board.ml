...
let opponent_of (player: player) : player =
  if equal_player player White then 
    Black 
  else 
    White
...
let set_board_pos_exn (board: 'a list list) ~(idx: int * int) ~(pos: 'a) : 'a list list =
  match idx with
  | (row, col) -> 
    if row < 0 || row >= 8 || col < 0 || col >= 8 then 
      raise (PosErr ("invalid pos in is_blocked_helper; " 
                     ^ "row: " 
                     ^ (string_of_int row) 
                     ^ " col: " 
                     ^ (string_of_int col)))
...
let rec is_blocked_helper (board: board) (ridx: int) (cidx: int) (next_ridx: int) (next_cidx: int) (rd: int) (cd: int) (curr_player: player): bool =
  let curr_pos = 
    (match get_board_pos board (ridx, cidx) with
     ...
     | None -> 
       raise (
         PosErr ("invalid pos in is_blocked_helper; " 
                 ^ "row: " 
                 ^ (string_of_int ridx) 
                 ^ " col: " 
                 ^ (string_of_int cidx))))
  in
  if (ridx = next_ridx) && (cidx = next_cidx) then
    (* If the piece at target position is of the same color, then the path is blocked *)
    (if (not (is_empty_pos curr_pos)) then 
       (equal_player (get_player_from_pos curr_pos) curr_player) 
     else 
       false)
  else
    match get_board_pos board (ridx, cidx) with
    ...
    | Some(Occupied(_)) -> 
      true
    | Some(Empty) -> 
      is_blocked_helper board (ridx+rd) (cidx+cd) next_ridx next_cidx rd cd curr_player
    | None -> 
      raise (
        PosErr ("invalid pos in is_blocked_helper; " 
                ^ "row: " 
...
let is_blocked (board: board) (ridx: int) (cidx: int) (next_ridx: int) (next_cidx: int) : bool =
  let rd = 
    if (Int.abs((next_ridx - ridx)) = 0) then 
      0 
    else 
      (next_ridx - ridx) / Int.abs((next_ridx - ridx)) 
  in
  let cd = 
    if (Int.abs((next_cidx - cidx)) = 0) then 
      0 
    else 
      (next_cidx - cidx) / Int.abs((next_cidx - cidx)) 
  in
  match get_board_pos board (ridx, cidx) with
  | Some(Occupied(c)) ->  
    is_blocked_helper board (ridx+rd) (cidx+cd) next_ridx next_cidx rd cd (get_player c)
  | Some(Empty) -> 
    raise (PosErr "position doesn't contain piece")
  | None -> 
    raise (PosErr "invalid pos")

let is_valid_pawn_move (board: board) (curr_player: player) (ridx: int) (cidx: int) (next_r: int) (next_c: int) : bool =
  if next_r < 0 || next_r > 7 || next_c < 0 || next_c > 7 then 
    false 
  else
    let next_pos = get_board_pos_exn board (next_r, next_c) 
    in
    if cidx = next_c then 
      is_empty_pos next_pos && 
      (((abs (ridx - next_r)) = 1) 
       || 
       (((abs (ridx - next_r)) = 2) 
        && (is_empty_pos (
            get_board_pos_exn board (
              next_r - (if next_r > ridx then 
                          1 
                        else 
                          (-1)), 
              next_c)))))
    else if not (((abs (ridx - next_r)) = 1) && ((abs (cidx - next_c)) = 1)) then 
      false
    else if is_empty_pos next_pos then 
      false
    else 
      not (equal_player (get_player_from_pos next_pos) curr_player)
...
let is_valid_king_move (board: board) (curr_player: player) (ridx: int) (cidx: int) (next_r: int) (next_c: int) : bool =
  ...
  if not ((abs(next_r - ridx) = 1 && abs(next_c - cidx) = 0) 
          ...
          || (abs(next_r - ridx) = 1 && abs(next_c - cidx) = 1)) 
  then 
    false 
  else
    match get_board_pos_exn board (next_r, next_c) with
...
let is_valid_queen_move (board: board) (ridx: int) (cidx: int) (next_r: int) (next_c: int) : bool =
  ...
  then 
    false
  else if not ((abs(next_r - ridx) > 0 && abs(next_c - cidx) = 0) 
               || (abs(next_r - ridx) = 0 && abs(next_c - cidx) > 0)
               || (abs(next_r - ridx) > 0 && abs(next_c - cidx) > 0 
                   && abs(next_r - ridx) = abs(next_c - cidx))) 
  then 
    false 
  else
...
let is_valid_rook_move (board: board) (ridx: int) (cidx: int) (next_r: int) (next_c: int) : bool =
  ...
  then 
    ...
    not ((abs(next_r - ridx) > 0 && abs(next_c - cidx) = 0) 
         || (abs(next_r - ridx) = 0 && abs(next_c - cidx) > 0)) 
  then 
    false 
  else
...
let get_possible_moves (board: board) (chess: chess) (ridx: int) (cidx: int) : (int * int) list =
  match chess with
  | King((player, _)) ->
    [(ridx+1, cidx); (ridx-1, cidx); (ridx, cidx+1); 
     (ridx, cidx-1); (ridx+1, cidx+1); (ridx+1, cidx-1); 
    ...
    |> List.filter 
      ~f:(
        fun idx -> 
          match idx with 
          | (next_r, next_c) -> 
            is_valid_king_move board player ridx cidx next_r next_c
      )
    ...
    |> List.filter 
      ~f:(
        fun idx -> 
          match idx with 
          | (next_r, next_c) -> 
            is_valid_queen_move board ridx cidx next_r next_c
      )
    ...
    |> List.filter 
      ~f:(
        fun idx -> 
          match idx with 
          | (next_r, next_c) ->
            is_valid_rook_move board ridx cidx next_r next_c
      )
    ...
    |> List.filter 
      ~f:(
        fun idx -> 
          match idx with 
          | (next_r, next_c) -> 
            is_valid_bishop_move board ridx cidx next_r next_c
      )

  | Pawn(White) -> 
    ([(ridx+1, cidx); (ridx+1, cidx+1); (ridx+1, cidx-1)]) 
    @ (
      if ridx = 1 then 
        [(ridx + 2, cidx)] 
      else 
    ...
    |> List.filter
      ~f:(
        fun idx -> 
          match idx with 
          | (next_r, next_c) -> 
            is_valid_pawn_move board White ridx cidx next_r next_c
      )
    ...
    |> List.filter 
      ~f:(
        fun idx -> 
          match idx with 
          | (next_r, next_c) -> 
            is_valid_pawn_move board Black ridx cidx next_r next_c
      )

  | Knight(player) -> 
    [(ridx+2, cidx+1); (ridx+2,cidx-1); 
     (ridx-2, cidx+1); (ridx-2,cidx-1);
     (ridx+1, cidx+2); (ridx+1,cidx-2); 
     (ridx-1, cidx+2); (ridx-1,cidx-2)]
    |> List.filter 
      ~f:(
        fun idx -> 
          match idx with 
          | (next_r, next_c) -> 
            is_valid_knight_move board player ridx cidx next_r next_c
      )
    ...
    ~f:(
      fun src accum pos -> 
        ...
        | Occupied(piece)-> 
          let (ridx, cidx) = src 
          in
          if equal_player curr_player (get_player piece) then 
            (List.map (get_possible_moves board piece ridx cidx) 
               ~f:(
                 fun dst -> 
                   (src, dst)
               )
...
let rec append_possible_moves_to_map (curr_piece: chess) (curr_map: pos list list list) (possible_moves: (int * int) list) : pos list list list =
  ...
  | move_pos :: ms -> 
    (let next_map = 
       append_possible_moves_to_map curr_piece curr_map ms 
     in
     match 
       (set_board_pos next_map move_pos 
          (Occupied(curr_piece) :: 
           match get_board_pos next_map move_pos with 
           | Some(t) -> 
             t 
           | None -> 
             raise (PosErr "can't find pos to append to"))
       )
     with
     | Some(map) -> 
       map
     | None ->
       raise (PosErr "Something wrong..."))
  | [] -> 
    curr_map
...
let add_next_step (board: board) (pos: pos) (ridx: int) (cidx: int) (curr_player: player) (curr_map: pos list list list) : pos list list list =
  ...
  | Empty -> 
    curr_map
  | Occupied(chess) -> 
    if not (equal_player (get_player chess) curr_player) then 
      curr_map
    else 
      get_possible_moves board chess ridx cidx 
      |> append_possible_moves_to_map chess curr_map
      ...
      ~f:(
        fun ridx r -> 
          List.mapi r 
            ~f:(
              fun cidx pos ->
                curr_map := add_next_step board pos ridx cidx curr_player (!curr_map); pos
            )
      ) 
  in
  !curr_map
          ...
          List.mapi r 
            ~f:(
              fun cidx pos -> 
                match pos with 
                | Occupied(King((p, _))) -> 
                  if equal_player p target_player then 
                    king_pos := (ridx, cidx); pos
                | _ -> 
                  pos 
            )
      )
...
let is_check (board: board) (curr_player: player) : bool =
  let next_step_map = get_next_step_map board curr_player 
  in
  let king_pos = find_king board (opponent_of curr_player) 
  in
  List.length 
    (match get_board_pos next_step_map king_pos with 
     | Some(l) -> l 
...
let is_checkmate (board: board) (curr_player: player) : bool =
  let all_moves = get_all_possible_moves board (opponent_of curr_player) 
  in
  let next_boards = List.map all_moves ~f:(fun (f, t) -> move_pure board f t) 
  in
  not(
    List.exists next_boards 
      ~f:(fun wrap -> 
          match wrap with
          | Some(next_board) -> 
            not (is_check next_board curr_player)
          | None -> 
            false
        )
  )
  ...
  | (rf, cf), (rt, ct) -> 
    if rf = rt && cf = ct then 
      ...
      (match (get_board_pos board f), (get_board_pos board t) with
       ...
       | Some(Empty), _ -> 
         false
       | Some(Occupied(chess)), _ -> 
         if not (equal_player (get_player chess) curr_player) then 
           false 
         else
           (match chess with
            | King((player, _)) -> 
              is_valid_king_move board player rf cf rt ct
            | Queen(_) -> 
              is_valid_queen_move board rf cf rt ct
            | Rook(_) -> 
              is_valid_rook_move board rf cf rt ct
            | Bishop(_) -> 
              is_valid_bishop_move board rf cf rt ct
            | Knight(player) -> 
              is_valid_knight_move board player rf cf rt ct
            | Pawn(player) -> 
              is_valid_pawn_move board player rf cf rt ct
           )
      )

let move (board: board) (curr_player: player) (f: int * int) (t: (int * int)) : (board * condition) =
  if not (validate board curr_player f t) then 
    (board, Fail("invalid move")) 
  else
    ...
    | Some(Occupied(curr_chess)) -> 
      (
        ...
        let next_pos = 
          (if not 
              (((equal_chess curr_chess (Pawn(White))) 
                || (equal_chess curr_chess (Pawn(Black)))) 
               && (rt = 0 || rt = 7)) 
           then 
             (match curr_chess with 
              | King(p, _) -> Occupied(King(p, true)) 
              | Rook(p, _) -> Occupied(Rook(p, true)) 
              | _ -> Occupied(curr_chess))
           else 
             Occupied(Queen(get_player curr_chess))) 
        in                    
        match set_board_pos board t next_pos with
        | Some(nb) ->   
          (match set_board_pos nb f Empty with
           | Some(nb1) ->  
             if (is_check nb1 (opponent_of curr_player)) then 
               (board, Fail("still checked after move")) 
             else if is_checkmate nb1 curr_player then 
               (nb1, Checkmate)
             else if is_check nb1 curr_player then
               (nb1, Check)
             else 
               (nb1, Normal)
           | None -> 
             (board, Fail("fail to set piece")))
        | None -> 
          (board, Fail("fail to set piece"))
      )
    | _ -> 
    ...
    (match (get_board_pos_exn board (7, 4)), (get_board_pos_exn board (7, 0)) with
     | Occupied(King(Black, false)), Occupied(Rook(Black, false)) -> 
       (if (is_valid_rook_move board 7 0 7 3) then 
          Some(
            set_board_pos_exn board ~idx:(7, 0) ~pos:Empty 
    ...
    (match (get_board_pos_exn board (0, 4)), (get_board_pos_exn board (0, 0)) with
     | Occupied(King(White, false)), Occupied(Rook(White, false)) -> 
       (if (is_valid_rook_move board 0 0 0 3) then 
          Some(
            set_board_pos_exn board ~idx:(0, 0) ~pos:Empty 
    ...
    (match (get_board_pos_exn board (7, 4)), (get_board_pos_exn board (7, 7)) with
     | Occupied(King(Black, false)), Occupied(Rook(Black, false)) -> 
       (if (is_valid_rook_move board 7 7 7 5) then 
          Some(
            set_board_pos_exn board ~idx:(7, 7) ~pos:Empty 
            |> set_board_pos_exn ~idx:(7, 4) ~pos:Empty
    ...
    (match (get_board_pos_exn board (0, 4)), (get_board_pos_exn board (0, 7)) with
     | Occupied(King(White, false)), Occupied(Rook(White, false)) -> 
       (if (is_valid_rook_move board 0 7 0 5) then 
          Some(
            set_board_pos_exn board ~idx:(0, 7) ~pos:Empty 
            |> set_board_pos_exn ~idx:(0, 4) ~pos:Empty
