...
struct
  ...
  let get_piece_value (piece: chess) : int =
    match piece with
    | Pawn(owner) -> 
      if equal_player Black owner then 
        100 
      else 
        -100
    | Knight(owner) -> 
  ...
  let eval (board: board) (curr_player: player) : value =
              ...
              List.fold col 
                ...
                ~f:(
                  fun accum2 pos -> 
                    ...
                    (match pos with 
                     | Empty -> (0) 
                     | Occupied(piece) -> get_piece_value piece)
                )
            )
          )
    in
    ...
    let curr_checkmate = 
      (
        if is_checkmate board curr_player then 
          (match curr_player with 
           | Black -> 100000 
           | White -> -100000) 
        else 
...
struct
  ...
  let rec eval_board (board: board) ~(curr_player: player) ~(curr_depth: int) ~(limit: int) : (Eval.value) =
    if is_checkmate board Black then
      Eval.min_value
    else if is_checkmate board White then 
      Eval.max_value
    else if curr_depth >= limit then 
      (Eval.eval board curr_player)
    else
      let all_moves = get_all_possible_moves board curr_player
      in
      let next_boards = List.map all_moves
          ~f:(
            fun (f, t) -> move board curr_player f t
          )
      in
      match curr_player with
      ...
      | Black  -> 
        ...
        List.fold next_board_scores 
          ~init:(Eval.min_value) 
          ~f:(
            fun curr_max curr_value -> 
              if Eval.compare curr_value curr_max > 0 then 
                curr_value else 
                curr_max
          )
      ...
      | White  -> 
        ...
        List.fold next_board_scores 
          ~init:(Eval.max_value) 
          ~f:(
            fun curr_min curr_value -> 
              if Eval.compare curr_value curr_min < 0 then 
                curr_value
              else 
                curr_min
          )
  ...
  let get_best_move (board: board) (curr_player: player) : move option =
    ...
    in
    if List.length next_boards = 0 then
      None
    else
      match curr_player with
      | Black -> 
        ...
        in
        if List.length next_board_scores = 0 then
          None 
        else
          let (p, _) = 
            List.fold next_board_scores
              ~init:(((-1, -1), (-1, -1)), Eval.min_value) 
              ~f:(
                fun (best_pair, curr_max) (curr_pair, curr_value) -> 
                  if Eval.compare curr_value curr_max > 0 then 
                    (curr_pair, curr_value)
                  else 
                    (best_pair, curr_max)
              )
          in 
      ...
      | White -> 
        ...
        if List.length next_board_scores = 0 then
          ...
          let (p, _) = 
            List.fold next_board_scores 
              ~init:(((-1, -1), (-1, -1)), Eval.max_value) 
              ~f:(
                fun (best_pair, curr_min) (curr_pair, curr_value) -> 
                  if Eval.compare curr_value curr_min < 0 then 
                    (curr_pair, curr_value)
                  else 
                    (best_pair, curr_min))
